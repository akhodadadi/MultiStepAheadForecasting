...
#===feature extraction===
if ifLoadFeatures:    
    df_train_pred=pd.read_csv(join(dataDir,'model2_trainData.csv'))
    df_eval=pd.read_csv(join(dataDir,'model2_evalData.csv'))
    df_test=pd.read_csv(join(dataDir,'model2_testData.csv'),
...
#    df_test.drop(['air_store_id','visit_date'],axis=1,inplace=True)    
    ...
    #===save derived data===
    df_train_pred.to_csv(join(dataDir,'model2_trainData.csv'),index=False)
    df_eval.to_csv(join(dataDir,'model2_evalData.csv'),index=False)
    df_test.to_csv(join(dataDir,'model2_testData.csv'),index=False)
    #===save derived data===
...
#                          subsample=.6, subsample_freq=1, 
#                          colsample_bytree=.7, reg_lambda=0, 
#                          silent=False)
...
if not modelSelection:
    ...
    bst=gbm.LGBMRegressor(boosting_type='gbdt', num_leaves=50, 
                  max_depth=-1, 
                  learning_rate=.005,n_estimators=2000,
                  subsample_for_bin=1000000, objective='rmse', 
                  subsample=.6, subsample_freq=1, 
                  colsample_bytree=.7, reg_lambda=0, 
                  silent=False)
    ...
    bst.fit(X=X_eval,y=Y_eval,
            eval_set=[(X_eval,Y_eval)],eval_names=['eval'],
            eval_metric=['rmse'],early_stopping_rounds=2000,
            feature_name=feature_names,
            categorical_feature=categorical_features)
    joblib.dump(bst,join(fittedModelDir,
                         'model2_nonCV_{}{}'.format(date,'.pkl'))) 
    ...
    fittedMdlPath='/home/arash/MEGA/MEGAsync/Machine Learning/'+\
                    'Kaggle/Recruit/Fitted models/model2_nonCV_{}.pkl'.\
                    format(date)
    ...
    df.to_csv(join(submissionsDir,'model2_{}.csv'.format(date)),index=False)
    #===make prediction for test set===         
